...
async function createCalendarEvent(args) {
    ...
    try {
        ...
        if (error instanceof Error) {
            ...
            debugLog('Unknown error:', JSON.stringify(error));
        }
        if (error instanceof Error) {
            throw new Error(`Failed to create event: ${error.message}`);
        }
        else {
            throw new Error('Failed to create event: An unknown error occurred');
        }
    }
}
// Tool handlers
    ...
    try {
        ...
        switch (name) {
            case "create_event": {
                ...
                const result = await createCalendarEvent(validatedArgs);

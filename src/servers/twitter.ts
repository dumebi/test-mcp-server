...
import { z } from "zod";
import { TwitterService } from '../providers/twitterProvider.js';
...
server.tool("get_tweet_by_id",
  ...
  async ({ tweetId }) => {
    ...
    return {
      content: [{ type: "text", text: JSON.stringify(tweet) }]
    };
  }
);
...
server.tool("like_tweet",
  ...
  async ({ tweetId }) => {
    ...
    return {
      content: [{ type: "text", text: JSON.stringify(likeResult) }]
    };
  }
);
...
server.tool("follow_user",
  ...
  async ({ targetUserId }) => {
    ...
    return {
      content: [{ type: "text", text: JSON.stringify(followResult) }]
    };
  }
);
...
server.tool("unfollow_user",
  ...
  async ({ targetUserId }) => {
    ...
    return {
      content: [{ type: "text", text: JSON.stringify(unfollowResult) }]
    };
  }
);
